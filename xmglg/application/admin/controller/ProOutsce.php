...
class ProOutsce extends Base {
    ...
    public function prodoAdd(Request $request) {
        ...
        // print_r($data);
        // exit;
        $res = db('prooutsce_x')->insert($data);
        if ($res) {
            $this->success("添加成功");
        } else {
            $this->error('添加失败');
    ...
    public function prodelete() {
        ...
        if (is_numeric($id) && $id > 0) {
            ...
            $suc = db('prooutsce+x')->where('id', 'eq', $id)->delete();
            if ($suc) {
                $this->success("删除成功");
            } else {
                $this->error("删除失败");
    ...
    public function proupdate() {
        $id = input('param.id');
        if (is_numeric($id) && $id > 0) {
            $list = db('prooutsce_x')->where('id', 'eq', $id)->select();
    ...
    public function proUpdateDo(Request $request) {
        ...
        // exit;
        $id = $data['id'];
        $res = db('prooutsce_x')->where('id', 'eq', $id)->update($data);
        if ($res) {
            $this->success("更新成功");
        } else {
            $this->error("更新失败");
